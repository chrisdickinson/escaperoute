...
Route.prototype.getTarget = function(base_path) {
    if(typeof(this.target) === 'string') {
        var full_path = base_path.split('.').concat(this.target.split('.')),
...
var Router = function(base_path, routes) {
...
Router.prototype.reverse = function(name, args, source) {
    ...
    for(var i = 0; i < this.length; ++i) {
        ...
        if(this.routes[i].name === name) {
            ...
            try {
                return target.reverse(name, args, source+target.re.source);
            } catch(err) {
                if(!(err instanceof NoReverseMatch)) {
                    throw err;
                }
    ...
    for(var i = 0; i < this.length; ++i) {
        ...
        if(match) {
            ...
            if(target instanceof Router) {
                ...
                return function() {
                    return target.apply({}, Array.prototype.slice.apply(arguments).concat(match.slice(1)));
                };
            }
        }
    }
    throw new NoMatch(path);
};
